...
export interface CardProps {
  ...
  isOpen: boolean;
  toggleOpen: () => void;
  handleSelect: (sector: string, itemName: string) => void;
}
...
const SectorCard: React.FC<CardProps> = ({
  ...
  return (
    <div>
      <Image
        src={imageSrc}
        alt={sectorName ? sectorName : "sector"}
        width={400}
        height={300}
        style={{ width: "400px", height: "300px", objectFit: "cover" }}
      />
      <div
        className={`bg-[#A6C0E6] [box-shadow:inset_0px_4px_4px_rgba(0,_0,_0,_0.25)] h-[50px] p-[10px] text-center font-bold text-[18px] relative cursor-pointer ${
          isOpen ? "!bg-[#102E6A] text-[white]" : ""
        }`}
        onClick={toggleOpen}
      >
        ...
        <svg
          ...
          className={`fill-[black] absolute top-0 right-0 mt-[5px] mr-[10px] mb-[0] ml-[0] ${
            isOpen ? "fill-[white]" : ""
          }`}
        >
          ...
          >
            {items.map((item, index) => (
              <div
                ...
                onClick={() => handleSelect(sectorName, item)}
              >
                {item}
              </div>
            ))}
